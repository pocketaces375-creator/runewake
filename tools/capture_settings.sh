#!/usr/bin/env bash
set -euo pipefail
# Settings screen capture: standard + wide.
# Usage: bash tools/capture_settings.sh
...
mkdir -p "$CAPTURE_DIR"
rm -f "$CAPTURE_DIR"/settings_test*.png "$CAPTURE_DIR"/settings_test*.meta.json
...
capture_one() {
    ...
    echo "=== Capture: settings_test${suffix} (${width}x${height}) ==="
    ...
    # Verify output
    if [ -f "$CAPTURE_DIR/settings_test${suffix}.png" ]; then
        local pw ph
        read pw ph <<< "$(python3 -c "
import struct
with open('${CAPTURE_DIR}/settings_test${suffix}.png','rb') as f:
    f.read(8)
    while True:
        ...
        if ct == b'IHDR':
            ...
            break
")"
        echo "  -> settings_test${suffix}.png: ${pw}x${ph}"
        if [ "$pw" -ne "$width" ] || [ "$ph" -ne "$height" ]; then
            echo "  WARNING: Expected ${width}x${height}, got ${pw}x${ph}" >&2
        fi
    else
        echo "  FAIL: settings_test${suffix}.png not produced" >&2
        rc=1
    fi
...
echo ""
echo "=== Settings Gate ==="
python3 "$ROOT/tools/capture_gate.py" settings_test settings_test_wide 2>&1 || {
    echo "FAIL: Settings gate failed" >&2
    exit 1
}
echo "Settings gate PASSED"