...
Uses 9-slice reconstruction (nineslice.py) so band thickness is uniform
and independent of the generated image. Frames whose window cannot be
detected are rejected entirely.
...
# ── Card rendering using nineslice ──────────────────────────────────
_CACHE: dict = {}


def make_card(frame_path: str,
              card_id: str, name: str,
              cost: int | None, atk: int | None, hp: int | None,
              size: tuple[int, int],
              weapon=False, suppressed=False,
              pips: tuple[int, int] | None = None) -> Image.Image | None:
    """Render one card at `size` using 9-slice border reconstruction.

    The border is built directly at `size` with band = BAND_FRAC * size[0].
    Art fills the detected window; the border is alpha-composited on top.
    Returns None if the source frame fails detection.
    """
    key = (frame_path, card_id, cost, atk, hp, size, weapon, suppressed, pips)
    if key in _CACHE:
        return _CACHE[key]

    bp = int(size[0] * BAND_FRAC)
    result = nineslice_frame(frame_path, size, band_px=bp)
    if result is None:
        return None
    ...
    if weapon:
        # Tint the whole card teal
        tint = Image.new("RGB", (BW, BH), (40, 120, 118))
        band = Image.new("L", (BW, BH), 255)
        ImageDraw.Draw(band).rectangle([x0, y0, x1, y1], fill=0)
        canvas = Image.composite(
            Image.blend(canvas.convert("RGB"), tint, 0.4).convert("RGBA"),
            canvas, band)
    ...
    if cost is not None:
        ...
        d.ellipse([cx - r, cy - r, cx + r, cy + r],
                  fill=(30, 25, 18), outline=GOLD_HI, width=7)
    ...
    nm = name.upper()
    d.text((BW // 2 + 3, ny - 58 + 3), nm, font=cz(48),
           fill=(0, 0, 0, 235), anchor="mm")
    ...
    if weapon and pips:
        ...
        for sx, col, val in [(x0 + 82, (176, 58, 48), atk),
                             (x1 - 82, (76, 138, 76), hp)]:
            d.rounded_rectangle([sx - 58, ny - 22, sx + 58, ny + 46],
                                radius=15, fill=col,
                                outline=(0, 0, 0, 210), width=4)
            d.text((sx, ny + 12), str(val), font=cz(48),
                   fill=(255, 255, 255), anchor="mm")

    if suppressed:
        d.text((BW // 2, BH // 2), "SUPPRESSED", font=cz(44),
               fill=(190, 195, 192), anchor="mm")
...
def plate(img: Image.Image, x, y, w, h, name, cur, mx, ca, cb):
    ...
    d.rounded_rectangle([x, y, x + w, y + h], radius=r,
                        fill=(20, 18, 15, 235), outline=BRONZE, width=2)
    fw = int((w - 6) * cur / mx)
    if fw > 0:
        bar = Image.new("RGBA", (fw, h - 6), (0, 0, 0, 0))
        ImageDraw.Draw(bar).rounded_rectangle([0, 0, fw - 1, h - 7],
                                               radius=r - 3, fill=cb)
        img.alpha_composite(bar, (x + 3, y + 3))
    d = ImageDraw.Draw(img, "RGBA")
    d.rounded_rectangle([x, y, x + w, y + h], radius=r, outline=GOLD, width=2)
    f = cz(int(h * 0.52))
    d.text((x + w // 2 + 2, y + h // 2 + 2), name.upper(), font=f,
...
def build(frame_path: str, out_path: str, label: str):
    ...
    board = board.resize((int(board.width * sc) + 1, int(board.height * sc) + 1),
                          Image.LANCZOS)
    img = board.crop(((board.width - W) // 2, (board.height - H) // 2,
                      (board.width - W) // 2 + W, (board.height - H) // 2 + H)
                     ).convert("RGBA")
    ...
    img = Image.composite(
        img,
        Image.new("RGBA", (W, H), (8, 7, 5, 255)),
        v.filter(ImageFilter.GaussianBlur(160)).point(lambda p: 55 + p * 200 // 255))
    ...
    for i in range(5):
        ...
        ImageDraw.Draw(rib).rounded_rectangle(
            [0, 0, CW + 19, py + CH - ey + 19], radius=18,
            fill=(201, 168, 76, 32), outline=(201, 168, 76, 85), width=2)
    ...
    def well(x, y):
        ...
        dl = ImageDraw.Draw(L)
        dl.rounded_rectangle([0, 0, CW, CH], radius=14,
                             fill=(12, 11, 9, 70),
    ...
    for i in range(5):
        x = x0 + i * (CW + GAP)
        if i < len(ENEMY):
            c = make_card(frame_path, *ENEMY[i], (CW, CH))
            if c is None:
                print(f"  REJECT: {frame_path} — make_card failed for enemy slot {i}")
                return None
            c, p = shadow(c)
    ...
    for i in range(5):
        x = x0 + i * (CW + GAP)
        if i == 2:
            c = make_card(frame_path, *MINE[0], (CW, CH))
            if c is None:
                print(f"  REJECT: {frame_path} — make_card failed for player slot")
                return None
            c, p = shadow(c)
    ...
    def arsenal(ax, ay, ids, nms, deck, barrow, pips, sup):
        ...
        dd.rounded_rectangle([ax, ay, ax + 356, ay + 176], radius=14,
                             fill=(18, 16, 13, 205), outline=BRONZE, width=3)
        wx = ax + 14
        for i, wid in enumerate(ids):
            w_card = make_card(frame_path, wid, nms[i], None, None, None,
                               (96, 140), weapon=True, suppressed=sup[i],
                               pips=pips[i])
        ...
        for j, (val, lab) in enumerate([(deck, "DECK"), (barrow, "BARROW")]):
            ...
            dd.text((ax + 293, cy + 48), lab, font=cz(15),
                    fill=(184, 168, 138), anchor="mm")

    plate(img, W - 500, 34, 380, 44, "The Wayfarer", 22, 30,
          (110, 36, 28), (168, 53, 42))
    arsenal(W - 380, 96,
            ["hol_x_the_black_barrow", "emb_x_the_last_ember"],
            ["Black Barrow", "Last Ember"], 22, 0, [(2, 3), (0, 3)], [False, True])
    arsenal(24, H - 286,
            ["vrd_x_heartwood_relic", "emb_x_the_last_ember"],
            ["Heartwood", "Last Ember"], 24, 0, [(1, 3), (0, 3)], [False, False])
    plate(img, 120, H - 84, 380, 44, "Trikzos", 21, 30,
          (46, 90, 46), (76, 138, 76))
    d = ImageDraw.Draw(img, "RGBA")
    for i in range(5):
        cx, cy = 132 + i * 34, H - 24
    ...
    for i, card in enumerate(HAND):
        t = i - (n - 1) / 2
        c = make_card(frame_path, *card, (HW, HH))
        if c is None:
            print(f"  REJECT: {frame_path} — make_card failed for hand slot {i}")
            return None
        c, p = shadow(c, blur=18, dy=10)
    ...
    # End turn button
    d = ImageDraw.Draw(img, "RGBA")
    d.rounded_rectangle([W - 260, H - 160, W - 40, H - 96],
                        radius=10, fill=(40, 36, 30, 245), outline=GOLD, width=3)
...
def main():
    ...
    if not made:
        print("no usable frames — all rejected", file=sys.stderr)
        sys.exit(1)
